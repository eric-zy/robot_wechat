...
-- 配置参数（优化版）
local config = {
	templatePath = "red_dot1.png|red_dot2.png",  -- 未读标记模板
	similarity = 0.85,               -- 图像识别相似度
	baseLeft = 50,                   -- 左边界
	baseRight = 800,                 -- 右边界
	baseTop = 200,                   -- 初始上边界
...
function processCurrentSegment()
	...
	if ret1 ~= -1 then
		...
		local findResult, x, y = findPicEx( 173,313,289,1774, "red_dot1.png|red_dot2.png|red_dot3.png|red_dot4.png|red_dot2_1.png|ret_dot_1_min.png|red_dot1_1.png,red_dot_1_1.png",0.9)
...
function processMessage()
	...
	if config.is_send_api == True then
		...
		if res.data and res.data.answer then
			...
			-- 检查answer是否包含收藏关键词
			local answerText = res.data.answer
			for key, keyword in pairs(config.collect_flow_keyword or {}) do
				if keyword and type(keyword) == "string" and string.find(answerText, keyword) then
					collect_content = keyword
					print("检测到收藏关键词:", keyword)
					break
				end
			end
		end
	...
	if msgPartsNum and msgPartsNum > 1 then
		print("两条以上的消息")
		common.sendMessages(msg_parts, send_img)
		sleep(1000)
		-- 确保返回到微信消息列表
