...
function M.run(options)
    ...
    while #pendingRecords > 0 and roundCount < maxRounds do
        ...
        if #selectedIds == 0 then
            ...
            -- 用 robot_code, channel_num 形式调用更新接口
            mass_send.updateMassSendRecordStatus(robot_code, channel_num, 1)
            break
        end
        ...
        mass_send.updateMassSendRecordStatus(nil, nil, nil, statusIds)
        sleep(1000)
        ...
        if #pendingRecords == 0 then
            print("所有联系人已发送完毕")
            -- 人员已全部匹配，更新接口传 robot_code, channel_num
            mass_send.updateMassSendRecordStatus(robot_code, channel_num, 1)
            -- 跳出循环，不再进入收藏选中流程
            break
        end
