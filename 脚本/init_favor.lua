...
function M.initFavor(robot_code, channel_num)
    ...
    -- 注：updateSyncStatus 在 processMediaResource / processLocationResource 末尾统一调用
        ...
        if not resourceChannelId then
            ...
            if not sendRes or not sendRes.success then
                ...
                if rtype == 1 or rtype == 2 then
                    ...
                    local ok = M.processMediaResource(sendRes.data, resource, rtype)
                    if ok then
...
-- @param rtype resource_type: 1=图片, 2=视频
-- @return boolean 是否成功
function M.processMediaResource(sendData, resource, rtype)
    local common = require("common")
    ...
    for retry = 1, maxRetry do
        ...
        if savePos then
            ...
            if not recheck then
				...
				sleep(1000)
				-- 检测是否回到聊天界面，最多重试3次
			local backToChat = false
			for retryBack = 1, 3 do
				ckSendMessage = ocr_start(209,205,844,338,"发消息")
				if ckSendMessage then
					print(string.format("processMediaResource: 已回到聊天界面（检测到发消息，第%d次检测）", retryBack))
					backToChat = true
					break
				end
				print(string.format("processMediaResource: 未检测到发消息，点击返回按钮（第%d/3次）", retryBack))
				randomTap(340,1950,1,1,"固定位置点击返回按钮")
				sleep(1500)
			end
			if not backToChat then
				print("processMediaResource: 3次重试后仍未回到聊天界面")
			end
				saveSuccess = true
                break
            end
    ...
    -- 6. 退出预览（图片/视频使用不同的退出方式）
    if rtype == 2 then
        -- 视频：通过findImage查找退出按钮图标
		randomSwipe(459,1153,5,5,"点击预览暂停唤醒退出图标")
		sleep(800)
        for exitRetry = 1, 5 do
            local quitIdx, quitX, quitY = findImage(74, 1635, 406, 1991, "id_video_quit_btn.png", 0.9)
            if quitIdx ~= -1 then
                randomTap(quitX + 20, quitY + 18, 5, 5, "点击退出视频预览")
                sleep(1500)
                -- 再次检测图标是否还存在
                local recheckIdx = findImage(74, 1635, 406, 1991, "id_video_quit_btn.png", 0.9)
                if recheckIdx == -1 then
                    print("processMediaResource: 已退出视频预览（退出图标已消失）")
                    break
                end
                print(string.format("processMediaResource: 第%d次退出图标仍存在，继续点击", exitRetry))
            end
        end
    else
        -- 图片：点击屏幕中间区域，检测回到聊天界面
        for exitRetry = 1, 5 do
            ...
            if backToChat then
                print("processMediaResource: 已退出图片预览，回到聊天界面")
                break
            end
    ...
    for retry = 1, 3 do
        ...
        if not recheck then
            print("processMediaResource: 发送按钮已消失，发送成功")
            break
        end
        print(string.format("processMediaResource: 发送按钮仍存在，第%d次重试", retry))
    end
    sleep(1000)

    -- 11. 发送文字消息（用于后续长按收藏），动态定位上一条消息
    -- 11.1 优先动态查找输入框位置（带重试：点击后检测输入法是否打开）
    local kbOpened = false
    for retry = 1, 3 do
        local idx, ix, iy = findImage(112, 1780, 194, 1891, "chat_box_left_btn.png|send_box1.png", 0.9)
        if idx ~= -1 then
            randomTap(ix + 180, iy + 35, 10, 10, "动态点击输入框")
        else
            randomTap(288, 1849, 20, 20, "固定位置点击输入框")
        end
        sleep(1000)
        -- 检测输入法是否打开（换行或搜索）
        local kbNl = ocr_start(692, 1685, 941, 1897, "换行")
        local kbSearch = ocr_start(692, 1685, 941, 1897, "搜索")
        if kbNl or kbSearch then
            print("processMediaResource: 输入法已打开")
            kbOpened = true
            break
        end
        print(string.format("processMediaResource: 第%d次输入法未打开，重试", retry))
    end
    if not kbOpened then
        print("processMediaResource: 多次重试后输入法仍未打开，继续流程")
    end
    ...
    if quickReply then
        ...
        -- 尝试粘贴（带重试：点击输入框唤醒输入法→重新定位→长按唤醒粘贴）
        local pasteOk = false
        for retry = 1, 5 do
            sleep(1000)
            -- 第1次定位：点击输入框唤醒输入法
            local inputres, inputx, inputy = findImage(112, 1780, 194, 1891, "chat_box_left_btn.png|send_box1.png", 0.9)
            if inputres ~= -1 then
                randomTap(inputx + 180, inputy + 35, 10, 10, "动态点击输入框(唤醒输入法)")
            end
            sleep(1000)
            -- 第2次定位：输入法弹出后输入框位置变化，重新定位并长按
            local reLocRes, reLocX, reLocY = findImage(94,1006,694,1948, "chat_box_left_btn.png|send_box1.png", 0.9)
			print("第2次定位：输入法弹出后输入框位置变化",reLocRes, reLocX, reLocY)
            if reLocRes ~= -1 then
                longTap(reLocX + 180, reLocY + 35)
                print(string.format("processMediaResource: 重新定位输入框，长按唤醒粘贴 pos=(%d,%d)", reLocX + 180, reLocY + 35))
            end
            sleep(1000)
            -- 查找粘贴选项
            ...
            print(string.format("processMediaResource: 第%d次未找到粘贴选项，重试", retry))
        end
        if not pasteOk then
	...
	local photoCheck = ocr_start(109,1264,902,1835,"相册")
	local musicCheck = ocr_start(95,1123,938,1919,"音乐")
	local fhCheck = ocr_start(59,1494,646,1978,"符号")
    if kbCheck or photoCheck or musicCheck or fhCheck then
        randomTap(141,726, 20, 20, "关闭输入法-点击空白处")
	...
	-- 614,1345
    local longPressX = lastMsgX - 100
    if longPressX < 614 then
        longPressX = 614
    end
...
function M.processLocationResource(sendData, resource)
    ...
    for retry = 1, 3 do
        ...
        if boxIdx ~= -1 then
            randomTap(boxX+240, boxY, 30, 5, "点击输入框唤醒输入法")
   ...
   common.closeInputMethodIfNeeded()
    ...
    for retry = 1, 3 do
		...
		print("查找添加标签位置",addTagPos)
        if addTagPos then
           -- randomTap(addTagPos[1], addTagPos[2], 5, 5, "点击收藏")
           -- print("processLocationResource: 已点击收藏")
            longTap(addTagPos[1], addTagPos[2])
            sleep(4000)

            -- 检测粘贴按钮（带重试：最多3次，每次间隔1s）
            local pastePos = nil
            for retryPaste = 1, 3 do
                pastePos = ocr_start(118, 194, 948, 445, "粘贴")
                if pastePos then
                    randomTap(pastePos[1], pastePos[2], 2, 1, "点击粘贴位置")
                    break
                end
                print("未检测到粘贴，第" .. retryPaste .. "次重试...")
                sleep(1000)
            end
...
function M.sendToFileHelper(robot_code, channel_num)
    ...
    if idx ~= -1 then
        randomTap(x+20, y+20, 5, 5, "图片识别点击搜索")
    else
