...
function C.backToHomeWithCheck(maxRetries)
    ...
    while retryCount < maxRetries do
        ...
        local wxIndex = findImage(144,1693,941,1938,"wx_logo_1.png|wxapp.png|wxapp_logo.png|wechat_thumb.png",0.9)
...
function C.changeToWX()
    ...
    local index, x, y = findImage(0,0,0,0,"wxapp_logo.png|wx_logo_1.png|wechat_thumb.png",0.8)
    if index == -1 then
	...
	while retryCount < maxRetry do
		...
		if inWxList then
			...
			if cancelPos then
				...
				-- 未检测到取消按钮，检测左下方是否有"微信"文字
				local wxTabPos = ocr_start(119,1761,360,1939,"微信")
				print("检测左下方微信位置",wxTabPos)
				if wxTabPos then
					randomTap(wxTabPos[1], wxTabPos[2]-20, 20, 10, "点击左下方微信")
					sleep(1000)
					-- 校验是否回到微信列表
					local backInWxList = ocr_start(374, 224, 678, 292, "微信")
					if backInWxList then
						print("点击微信后已回到微信列表界面")
						return true
					else
						print("点击微信后未回到列表，等待1s再次检测点击")
						sleep(1000)
						wxTabPos = ocr_start(119,1761,360,1939,"微信")
						if wxTabPos then
							randomTap(wxTabPos[1], wxTabPos[2]-20, 20, 10, "再次点击左下方微信")
							sleep(1000)
						end
					end
				end
...
function C.openID(is_return_list_check)
    ...
    local maxRetry = 3
    local retryCount = 0
    local targetMark = config.id_ai_project_config.mark
    local targetName = config.id_ai_project_config.name
    local targetReceive = config.id_ai_project_config.receive_name
    while retryCount < maxRetry do
        retryCount = retryCount + 1
        if retryCount > 1 then
            sleep(1000)  -- 仅重试时等待
        end
        -- 一次OCR扫描替换原来的3次，避免重复大区域扫描
        local foundMark, foundName, foundReceive = false, false, false
        local boxes = ocr_start_with_boxes(115, 379, 908, 1728)
        for _, item in ipairs(boxes) do
            local words = item.words or ""
            if not foundMark and targetMark and string.find(words, targetMark) then
                foundMark = true
            end
            if not foundName and targetName and string.find(words, targetName) then
                foundName = true
            end
            if not foundReceive and targetReceive and string.find(words, targetReceive) then
                foundReceive = true
            end
            if foundMark and foundName and foundReceive then
                break  -- 三个都找到了，提前退出
            end
        end
        if foundMark or foundName or foundReceive then
            print("已在智企ID")
            if is_return_list_check then
                if foundName or foundReceive then
        ...
        if next(chat_pos_arr) == nil then
            ...
            while innerRetry < maxInnerRetry and not copyFound do
                ...
                if rey ~= -1 then
                    ...
                    -- 返回ID消息列表（最多重试10次，兜底盲点）
                    local bakRetry = 0
                    local maxBakRetry = 10
                    local bak_msg_list_ret = -1
                    local bx, by = -1, -1

                    while bak_msg_list_ret == -1 and bakRetry < maxBakRetry do
                        bakRetry = bakRetry + 1
                        sleep(1000)
                    ...
                    if bak_msg_list_ret == -1 then
                        print("检测返回按钮超时，盲点左上角返回")
                        randomTap(130, 255, 10, 5, "盲点返回ID消息列表")
                        sleep(500)
                    end
...
function C.addWXFriendSingleAttempt(op_start, tagName)
	...
	while inputRetry < 3 and not inputBoxOpened do
		...
		-- 验证输入法是否打开（检测换行或搜索）
		sleep(1000)
		local newlineRes = ocr_start(689,1542,977,1940, "换行")
		local searchRes = ocr_start(689,1542,977,1940, "搜索")
		local fhRes = ocr_start(689,1542,977,1940, "搜索")
		local clearRes = ocr_start(689,1542,977,1940, "清空")
		print("检测输入法是否打开 - 换行:", newlineRes, "搜索:", searchRes)
		if newlineRes or searchRes or fhRes or clearRes then
			print("输入法已打开")
			inputBoxOpened = true
		else
	...
	while pasteRetry < maxPasteRetry and not pasteSuccess do
		...
		if index ~= -1 then
			...
			if accountPos then
				...
				longTap(accountPos[1], accountPos[2])
			else
...
function C.getIDMessages()
	sleep(500)
    for _=1,2 do
        local arr = findPicAllPoint(209,244,398,1819,config.id_ai_project_config.logo,0.9)
		print("查找图标数组",arr)
...
function C.backToIDMessageList()
    local result = optimizedWait(function()
        local idx, x, y = findImage(0,0,200,350,"msg_bak.png|msg_back_1.png",0.9)
        if idx~=-1 then
			print("backToIDMessageList: 图片匹配到返回按钮，使用图片点击")
    ...
    if not result then
        print("backToIDMessageList: 图片未匹配到返回按钮，使用固定位置点击")
        randomTap(128,264, 3, 5, "固定位置返回ID列表")
        sleep(800)
        return true
    end
    return true
...
-- ==================== 主动联系未支付用户模块 ====================
C.reminderConfig = {
    next_url = config.reminder_next_url,
    send_url = config.reminder_send_url,
}
...
function C.handleReminder(options)
    ...
    for retrySearch = 1, 3 do
        ...
        if not wxStill then
            print("搜索按钮点击成功（微信已消失）")
            break
        end
        print("微信仍存在，第" .. retrySearch .. "次重试点击搜索...")
    end
    sleep(2000)
...
-- ==================== 发送多条微信消息 ====================
-- 发送多条微信消息
function C.sendMessages(msg_parts, send_img, collect_content)
	local op_start = os.time()
	-- 打开id
	-- 进入聊天界面
...
--	end
	...
	if collect_content and collect_content ~= "" then
		print("触发收藏转发流程(多条消息)，关键词:", collect_content)
		C.handleCollectForward(collect_content)
	end
...
function C.handleCollectForward(keyword)
	...
	-- 等待聊天界面稳定
	sleep(1000)
	-- 步骤0:点击+号（最多重试5次，检测输入法是否遮挡）
	local index1 = -1
	local x1 = -1
	local y1 = -1
	for retryPlus = 1, 5 do
		index1,x1,y1 = findImage(824,1100,947,1930,"wx_plus.png|wx_plus_1.png",0.9)
		print(string.format("检测+号位置结果（第%d/5次）", retryPlus), index1, x1, y1)
		...
		-- 检测输入法是否打开
		local keyboardOpen = ocr_start(767, 1583, 961, 1900, "换行|空格|符号")
		if keyboardOpen then
			print("检测到输入法打开，点击返回关闭输入法")
			randomTap(55, 77, 3, 3, "点击返回关闭输入法")
			sleep(1000)
		else
			print("未检测到输入法，点击空白区域重试")
			randomTap(500, 1000, 50, 50, "点击空白区域")
			sleep(800)
		end
	end
	if index1 ~= -1 then
	...
	keywordPos = nil
	local prevText = ""
	local maxPage = 20  -- 最多翻20页防止死循环
	for page = 1, maxPage do
		sleep(1500)
		keywordPos = ocr_start(96, 515, 913, 1914, keyword)
		print(string.format("查找关键词%s结果（第%d页）", keyword, page), keywordPos)
		
		if keywordPos then
			break
		end
		
		-- 获取当前页全部OCR文字，用于判断是否最后一页
		local currText = ""
		local allBoxes = ocr_start_with_boxes(96, 515, 913, 1914)
		if allBoxes then
			for _, b in ipairs(allBoxes) do
				currText = currText .. (b.words or "")
			end
		end
		
		-- 与上一页对比相似度
		if prevText ~= "" and currText ~= "" and #prevText > 0 then
			local sameCount = 0
			local totalLen = math.max(#prevText, #currText)
			local minLen = math.min(#prevText, #currText)
			for j = 1, minLen do
				if prevText:sub(j, j) == currText:sub(j, j) then
					sameCount = sameCount + 1
				end
			end
			local sim = sameCount / totalLen
			print(string.format("第%d页与上一页相似度: %.2f%%", page, sim * 100))
			if sim >= 0.95 then
				print("已到达最后一页（相似度>=95%），停止翻页")
				break
			end
		end
		prevText = currText
		...
		-- 滑动下拉翻页
		print(string.format("第%d页未找到关键词，滑动下拉翻页...", page))
	...
	if keywordPos then
		print("点击"..keyword.."位置")
		randomTap(keywordPos[1],keywordPos[2]-50,5,5,"点击分享"..keyword)
		sleep(1500)
		...
		print("警告：翻完所有收藏页仍未找到关键词:", keyword)
	end
	--回到微信列表
	sleep(3000)
